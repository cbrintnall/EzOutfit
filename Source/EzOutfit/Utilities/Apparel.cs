...
public static class ApparelUtilities
{
    ...
    public static bool IsUpsetting(this Apparel apparel) =>
        ...
        || (
            ModSettings.DropAllIncludesTattered.Value
            && apparel.NormalizedHitpoints()
                <= Mathf.Max(
                    ThoughtWorker_ApparelDamaged.MinForFrayed,
                    ThoughtWorker_ApparelDamaged.MinForTattered
                )
        );
}
